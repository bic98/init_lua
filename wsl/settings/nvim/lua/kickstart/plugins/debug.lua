...
return {
  ...
  config = function()
    ...
    local debugpy_path = debugpy_venv .. (vim.fn.has 'win32' == 1 and '/Scripts/python.exe' or '/bin/python')
    ...
    local function python_path()
      ...
      if venv then
        local candidate = venv .. (vim.fn.has 'win32' == 1 and '/Scripts/python.exe' or '/bin/python')
        if vim.fn.executable(candidate) == 1 then
          return candidate
        end
      end
      for _, command in ipairs { 'python3', 'python', 'py' } do
        local candidate = vim.fn.exepath(command)
        if candidate ~= '' then
          return candidate
        end
      end
