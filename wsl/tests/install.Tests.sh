...
case_dir="$(mktemp -d "$temp_root/init-lua-wsl-test.XXXXXX")"

if grep -Eq '^[[:space:]]*(sudo|apt|apt-get|brew|dnf|pacman|choco|winget|scoop)([[:space:]]|$)' "$platform_root/install.sh"; then
  printf '%s\n' 'FAIL: WSL installer must not invoke a package manager or sudo' >&2
  exit 1
fi
...
grep -Fq 'PASS: WSL dependency manifest is valid' <<< "$dependency_check"

cleanup() {
  [[ "$(dirname -- "$case_dir")" == "$temp_root" ]] || return 1
  [[ "$(basename -- "$case_dir")" == init-lua-wsl-test.* ]] || return 1
  rm -rf -- "$case_dir"
}
trap cleanup EXIT
...
if env \
  ...
  bash "$platform_root/scripts/verify.sh" \
    ...
    --skip-tools \
    --skip-plugins \
    --skip-tmux >/dev/null 2>&1; then
  printf '%s\n' 'FAIL: strict WSL verifier accepted a missing managed theme' >&2
  exit 1
fi

printf '%s\n' 'PASS: WSL isolated install, idempotency, and strict mismatch rejection'
