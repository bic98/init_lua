...
function Merge-ProfileItems {
    ...
    $sourceGuids = [System.Collections.Generic.HashSet[string]]::new([System.StringComparer]::OrdinalIgnoreCase)
    $sourceNames = [System.Collections.Generic.HashSet[string]]::new([System.StringComparer]::OrdinalIgnoreCase)
    foreach ($profile in $Source) {
        ...
        $null = $sourceGuids.Add($guid)
        $null = $sourceNames.Add($name)
    }
    ...
    foreach ($profile in $Existing) {
        ...
        $guid = [string](Get-PropertyValue -InputObject $profile -Name "guid")
        $name = [string](Get-PropertyValue -InputObject $profile -Name "name")
        if ($sourceGuids.Contains($guid) -or $sourceNames.Contains($name)) {
            continue
        }
        $merged.Add($profile)
    }
...
foreach ($target in $targets) {
    ...
    $settings = if ($targetExists) {
        Get-Content -Raw -LiteralPath $resolvedTarget | ConvertFrom-Json -Depth 100
    } else {
    ...
    $backupPath = if ($targetExists) { "$resolvedTarget.init_lua_backup_$timestamp" } else { $null }
    ...
    if ($PSCmdlet.ShouldProcess($resolvedTarget, $operation)) {
