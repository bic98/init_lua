...
마지막에 `pwsh -NoProfile -File .\window\scripts\Test-TerminalEnvironment.ps1 -Strict`를 실행한다. FAIL을 원인별로 수정하고 최대 3회 재검증한다. 모든 필수 검사가 PASS일 때만 RESULT: MATCHED라고 보고한다. 하나라도 남으면 RESULT: BLOCKED이며 성공이라고 말하지 않는다.
```
