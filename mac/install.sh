...
usage() {
  cat <<'EOF'
Usage: bash mac/install.sh [options]
  --shell zsh|bash       Select the managed shell profile
  ...
  --skip-font            Do not install JetBrainsMono Nerd Font
  --skip-nvim            Do not synchronize the macOS Neovim files
...
if [ "$skip_oh_my_posh" -eq 1 ] || [ "$skip_dependencies" -eq 1 ] || [ "$skip_claude" -eq 1 ] ||
   [ "$skip_font" -eq 1 ] || [ "$skip_nvim" -eq 1 ] || [ "$skip_nvim_plugins" -eq 1 ] ||
   [ "$skip_shell" -eq 1 ] || [ "$skip_tmux" -eq 1 ]; then
  partial_run=1
fi
...
case "$shell_name" in
  bash|zsh)
    ;;
  *)
    echo "ERROR: Supported macOS shells are zsh and bash, not '$shell_name'." >&2
    exit 1
    ;;
esac
...
font_is_installed() {
  [ -d "$HOME/Library/Fonts" ] && find "$HOME/Library/Fonts" -maxdepth 1 -type f \
    \( -iname 'JetBrainsMono*NerdFont*.ttf' -o -iname 'JetBrainsMono*NF*.ttf' \) -print -quit | grep -q .
}

echo
echo "=== init_lua macOS configuration ==="
echo "The declared macOS desired state will be installed and verified."

echo
echo "[1/8] Required tools"
...
echo
echo "[3/8] Neovim plugins and Mason tools"
...
echo
echo "[5/8] JetBrainsMono Nerd Font"
if font_is_installed; then
  echo "  JetBrainsMono Nerd Font already installed."
elif [ "$skip_font" -eq 1 ]; then
  echo "  Font installation skipped."
elif command -v oh-my-posh >/dev/null 2>&1; then
  oh-my-posh font install JetBrainsMono
else
  echo "  Font was not installed because Oh My Posh is unavailable."
fi

echo
echo "[6/8] $shell_name profile"
...
[ "$skip_font" -eq 1 ] || font_is_installed || { echo "  FAIL: JetBrainsMono Nerd Font"; failures=$((failures + 1)); }
...
if [ "$failures" -gt 0 ]; then
  echo "ERROR: init_lua macOS configuration failed $failures basic check(s)." >&2
  exit 1
fi
...
[ "$skip_font" -eq 1 ] && verification_args+=(--skip-font)
bash "$script_dir/scripts/verify.sh" "${verification_args[@]}"
if [ "$partial_run" -eq 1 ]; then
  echo "init_lua macOS partial run complete; RESULT is not MATCHED."
else
  echo "init_lua macOS configuration complete. Restart the shell to load it."
fi
